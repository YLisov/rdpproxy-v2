...
- `routes/services_mgmt.py`: статус сервисов + `GET /api/admin/services/health` — TCP/HTTP health-check всех сервисов (postgres, redis, portal, rdp-relay, haproxy, metrics) с latency. HAProxy проверяется через stats Unix-socket (`/var/run/haproxy/admin.sock`, команда `show info`) вместо TCP-подключения к порту 443, чтобы не генерировать ошибки SSL handshake.
