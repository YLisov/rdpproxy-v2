...
## Итерация #40
**Дата**: 2026-04-11
**Запрос**: Баг-фикс — установщик не получает LE-сертификат за NAT без hairpin

### Действие 40.1
**Описание**: Удалена функция `check_port80()` (бывшие строки 275-293) из `deploy/install.sh`. Функция запускала временный HTTP-сервер и обращалась к нему через curl по доменному имени; за NAT без hairpin это всегда давало таймаут, в результате certbot не запускался.
Удалён интерактивный цикл `while true` с повторными проверками и ручным вводом (бывшие строки 296-309).

### Действие 40.2
**Описание**: Блок «8. Certificate» заменён на упрощённую логику:
- Локальная проверка занятости порта 80 через `ss -tlnp` (если занят — сразу self-signed).
- При свободном порте — прямой запуск `certbot` (standalone, HTTP-01 challenge). Certbot сам является лучшей проверкой доступности порта извне.
- Если certbot не получил сертификат — fallback на self-signed с предупреждением.
- Убрана зависимость от `python3 http.server`, убрана интерактивность (retry-промпт).
